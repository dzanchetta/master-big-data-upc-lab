...
Todo el codigo abajo es para el pre-processing de los datos:
## 
```{r}
library(dplyr)
library(tidyr)
...
#Aplicando PCA nos fijamos que no hay codos que nos digan con certitumbre cuantos componentes significativas hay. Se nota que hay un pequeño codo convexo a un valor aproximado a 15 pero no se podría decir con toda certeza que esta seria la cantidad de componentes significativos
library(FactoMineR)
...
#Definimos el numero de componentes significativas igual a 25 pues acumula 90,08% de dimensiones significativas 
nd = 25 
...
# El codigo abajo es para aplicar los algoritmos de clasificación linear:
```{r}
set.seed(17)
...
# Logistic Regression.
#Aplico GLM usando binomial. Resultado: Estimates são os coeficientes. O que importa que é quanto mais asteriscos haja, menos relevante será a variavel - Treina o modelo
musk.glm <- glm (class ~ ., data = Musk[learn,], family = "binomial")
summary(musk.glm)

# Enseguida, intentamos simplificar un poco el modelo quitando las variables no significativas. Utilizamos función de step().
